...
export default function Home() {
  ...
  const content = (
    <div className="h-full lg:p-1">
        ...
        {filters.map((f) => (
          <button
            ...
            className={`px-3 py-1 rounded-lg border border-white/20 transition-colors duration-200 ${
              selectedFilter === f.value ? "bg-white/30" : "bg-white/10 hover:bg-white/20"
            }`}
          >
            {f.label}
          </button>
      ...
      <div className={`${screenType === 'mobile' ? 'block' : 'hidden'} space-y-4 px-2 pb-6`}>
        {[
          { icon: "/images/main_dashboard/followersIcon.svg", title: "Followers", count: "1.4K", chart: <FollowersChart /> },
          { icon: "/images/main_dashboard/connectionsIcon.svg", title: "Connections", count: "900", chart: <ConnectionsChart /> },
          { icon: "/images/main_dashboard/commentsIcon.svg", title: "Comments", count: "35", chart: <CommentsDotPlot /> }
        ].map((item, idx) => (
          <div key={idx} className="w-full bg-black/60 backdrop-blur-md border border-white/20 rounded-xl shadow-lg p-4 mb-10">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                <Image src={item.icon} alt={`${item.title} Icon`} width={20} height={20} />
        ...
        <div className="w-full bg-black/60 backdrop-blur-md border border-white/20 rounded-2xl p-4">
          <div className="text-center mb-3">
            ...
            <p className="text-xs text-white/70">Optimize your outreach by tracking which targets drive the most engagement.</p>
          </div>
          <div className="space-y-3">
            {users.map((user, i) => (
              <div key={i} className="border border-white/10 rounded-xl px-4 py-3 bg-black/30">
                <div className="flex items-center justify-between mb-2 text-xs text-white/60">
                  <span>No: {String(i + 1).padStart(2, "0")}</span>
                  <span>Latest: —</span>
                </div>
                <div className="flex items-center space-x-3 mb-1">
                  <Image src="/images/topbar/userIcon.svg" alt={user.name} width={30} height={30} className="rounded-full" />
                  <div>
      ...
      <div className={`${screenType === 'mobile' ? 'hidden' : 'grid'} grid-cols-3 auto-rows-min gap-6`}>
        {[
          { icon: "/images/main_dashboard/followersIcon.svg", title: "Followers", count: "1.4K", chart: <FollowersChart /> },
          { icon: "/images/main_dashboard/connectionsIcon.svg", title: "Connections", count: "900", chart: <ConnectionsChart /> },
          { icon: "/images/main_dashboard/commentsIcon.svg", title: "Comments", count: "35", chart: <CommentsDotPlot /> }
        ].map((item, idx) => (
          <div key={idx} className="w-full flex flex-col bg-black/60 backdrop-blur-md border border-white/20 rounded-xl shadow-lg p-4">
        ...
        <div className="col-span-3 flex flex-col bg-black/60 backdrop-blur-md border border-white/20 rounded-2xl p-6">
          <div className="flex flex-col items-center gap-0">
            <span className="text-xl font-semibold text-center">Comments Fusion Performance Tracker</span>
            <span className="text-sm text-center">Optimize your outreach by tracking which targets drive the most engagement.</span>
          </div>
          <div className="grid grid-cols-5 gap-4 mt-5 mb-2 px-4 text-sm font-medium">
            <span>No:</span><span>Name:</span><span>Comments count:</span><span>Impressions:</span><span>Latest comment date:</span>
          </div>
          <div className="flex-1 overflow-y-auto space-y-2">
            {users.map((user, i) => (
              <div key={i} className="grid grid-cols-5 gap-4 items-center px-4 border-t border-white/20 py-2">
                <span>{String(i + 1).padStart(2, "0")}</span>
                <div className="flex items-center space-x-3">
                  <img src="/images/topbar/userIcon.svg" alt={`${user.name} Icon`} className="w-8 h-8 rounded-full" />
                  <div className="text-sm">
                    <p className="font-medium">{user.name}</p>
                    <p className="text-xs text-gray-300">{user.followers} followers</p>
                  </div>
                </div>
                <span>0</span><span>0</span><span></span>
              </div>
            ))}
          </div>
